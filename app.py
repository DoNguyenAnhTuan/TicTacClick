...
socketio = SocketIO(app, cors_allowed_origins="*")  # CORS trong LAN
...
def check_winner(b):
    ...
    # chéo (chỉ thêm 1 lần, không lặp trong vòng for)
...
def select_map():
    ...
    current_map = mid
    reset_board()
    socketio.emit("board_update", {"board": board, "message": f"Đã chọn {MAPS[mid]['name']}."})
...
def move():
    ...
    if game_over:
        msg = "Ván đã kết thúc. Bấm Reset để chơi lại."
        socketio.emit("toast", {"message": msg, "error": True})
        return render_play(message=msg, error=True, last_player=player)

    if player not in ("X", "O"):
        msg = "Người chơi không hợp lệ."
        socketio.emit("toast", {"message": msg, "error": True})
        return render_play(message=msg, error=True, last_player=player)
    ...
    if result in ("X", "O"):
        msg = f"{result} thắng! 🎉"
        socketio.emit("board_update", {"board": board, "message": msg})
        game_over, winner = True, result
        return render_play(message=msg, error=False, last_player=player)
    elif result == "draw":
        msg = "Hoà (số quân bằng nhau)."
        socketio.emit("board_update", {"board": board, "message": msg})
...
def reset():
    if not current_map:
        return render_select()
    reset_board()
    socketio.emit("board_update", {"board": board, "message": "Đã reset bàn cờ."})
    return render_play(message="Đã reset bàn cờ.", error=False, last_player="X")

@socketio.on('connect')
def on_connect():
    emit("board_update", {"board": board, "message": ""})
